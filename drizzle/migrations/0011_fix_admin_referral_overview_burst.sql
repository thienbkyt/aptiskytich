...
begin
  ...
  select jsonb_build_object(
    ...
    'burst', (
      select coalesce(jsonb_agg(jsonb_build_object('user_id', b.uid, 'name', b.nm, 'n', b.n)), '[]'::jsonb)
      from (
        select e.referrer_user_id as uid, coalesce(p.display_name,'Học viên') as nm, count(*) as n
        from public.referral_earnings e
        left join public.profiles p on p.user_id = e.referrer_user_id
