...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Provider + voice + model are part of the cache path so switching voices or
    // models never serves audio generated by the previous configuration. Old cache stays intact.
    const provider = surface === "exam" ? "elevenlabs" : "openai";
    const voiceId = surface === "exam" ? ELEVENLABS_VOICE_ID : "onyx";
    const modelId = surface === "exam" ? "eleven_multilingual_v2" : "openai/gpt-4o-mini-tts";
    const cacheKey = buildCacheKey(text, lang, `${provider}|${voiceId}`, modelId);
    const safeModelId = modelId.replace(/\//g, "_");
    const path = `${lang}/${provider}_${voiceId}_${safeModelId}/${cacheKey}.mp3`;
