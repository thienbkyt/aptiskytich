...
Deno.serve(async (req) => {
  ...
  const rebuilt = await buildWritingPayloadFromDb(admin, result as any);
  if (!rebuilt) return json({ error: "submission_not_rebuildable" }, 400);
  const { partType, payload, gradingSessionId } = rebuilt;
