...
AS $function$
DECLARE
  r record; v_sent int := 0; v_cap int := 150;
...
BEGIN
  ...
  ---------- MAIL 1: D+1 (chi user tao tu 15/09) ----------
  FOR r IN
  ...
  ---------- MAIL 2: D+2 (ap dung ca backlog tu 01/09) ----------
  FOR r IN
    SELECT u.id AS user_id, u.email, u.created_at
    FROM auth.users u
    WHERE u.email IS NOT NULL AND btrim(u.email) <> ''
...
REVOKE ALL ON FUNCTION public.send_signup_nurture_emails() FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.send_signup_nurture_emails() TO service_role;

SELECT cron.schedule('signup-nurture-emails', '15 * * * *', $$select public.send_signup_nurture_emails();$$);