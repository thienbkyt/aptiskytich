...
BEGIN
  ...
  IF v_code.kind <> 'standalone' THEN
    RETURN jsonb_build_object('ok', false, 'reason', 'checkout_only', 'gift_days', 0, 'gift_ai_credits', 0, 'message', 'Mã này dùng khi thanh toán — vào trang bảng giá và nhập mã trước khi chọn gói nhé.');
  END IF;
