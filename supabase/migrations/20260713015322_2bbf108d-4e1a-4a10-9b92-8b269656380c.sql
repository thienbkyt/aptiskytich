-- Tighten storage read policies to enforce exam_sets.access_tier gating.
    ...
    OR EXISTS (
      ...
      WHERE eq.audio_url IS NOT NULL
        AND (
          ...
          OR eq.audio_url LIKE '%' || storage.objects.name
          OR storage.objects.name = split_part(eq.audio_url, '/', -1)
          OR storage.objects.name LIKE '%' || split_part(eq.audio_url, '/', -1)
        )
    ...
    OR EXISTS (
      ...
      WHERE eq.image_url IS NOT NULL
        AND (
          ...
          OR eq.image_url LIKE '%' || storage.objects.name
          OR storage.objects.name = split_part(eq.image_url, '/', -1)
          OR storage.objects.name LIKE '%' || split_part(eq.image_url, '/', -1)
        )
        AND public.tier_rank(public.user_tier(auth.uid()))
            >= public.tier_rank(COALESCE(es.access_tier, 'pro'))
    )
