...
BEGIN
  ...
  -- Writing + Speaking AI grading share ONE pooled allowance.
  SELECT COUNT(DISTINCT COALESCE(ref_id, id::text))
    INTO v_used
    FROM public.feature_usage
   WHERE user_id = v_uid
     AND (
       (p_key IN ('ai_grading_writing','ai_grading_speaking')
          AND feature_key IN ('ai_grading_writing','ai_grading_speaking'))
       OR (p_key NOT IN ('ai_grading_writing','ai_grading_speaking')
  ...
  IF p_key IN ('ai_grading_writing','ai_grading_speaking') THEN
    -- Số dư = tổng grant còn hạn TRỪ số lượt đã trả bằng quỹ.
    -- Cách đếm phải GIỐNG HỆT v_used ở trên: COUNT(DISTINCT COALESCE(ref_id, id::text)).
    -- Đổi cách đếm ở một nơi mà quên nơi kia sẽ làm quỹ lệch tiền.
    v_credits := GREATEST(0,
      COALESCE((SELECT SUM(amount) FROM public.ai_credit_grants
                 WHERE user_id = v_uid
                   AND (expires_at IS NULL OR expires_at > now())), 0)
      -
      COALESCE((SELECT COUNT(DISTINCT COALESCE(ref_id, id::text)) FROM public.feature_usage
                 WHERE user_id = v_uid AND paid_by_credit), 0)
    );
    v_would_use := (v_used >= COALESCE(v_cap, 0)) AND v_credits > 0;
  END IF;

  RETURN jsonb_build_object(
    'allowed', (v_used < COALESCE(v_cap, 0)) OR v_would_use,
    'reason', CASE
                WHEN v_used < COALESCE(v_cap, 0) THEN 'within_quota'
                WHEN v_would_use THEN 'within_credits'
                ELSE 'quota_exceeded'
              END,
