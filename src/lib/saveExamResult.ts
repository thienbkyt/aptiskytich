...
export async function saveExamResult(opts: SaveExamResultOpts): Promise<string | null> {
  ...
  try {
    ...
    // Reading/Listening insert a new row per submit. A standalone part does not
    // have a fullTestSessionId, and a remounted flow may have a different one, so
    // session id cannot be part of this guard. The same user + exam set within
    // five minutes is one submission; older attempts are deliberate retakes.
    const dedupSkills = new Set(["reading", "listening"]);
    if (opts.examSetId && dedupSkills.has(opts.skill)) {
      try {
        ...
        if (prev?.id) {
          console.info("[saveExamResult] recent Reading/Listening submission found; updating existing row", {
            skill: opts.skill,
            examSetId: opts.examSetId,
            previousResultId: prev.id,
            previousCreatedAt: prev.created_at,
            incomingSessionId: opts.fullTestSessionId ?? null,
          });
