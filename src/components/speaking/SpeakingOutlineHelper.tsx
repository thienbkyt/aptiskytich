...
export default function SpeakingOutlineHelper({ outlineB1, outlineB2, currentQuestion = 0 }: Props) {
  ...
  return (
    <div className="p-4">
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        {outline && (
          <button
            type="button"
            onClick={randomize}
            className="flex items-center gap-1 rounded-full border border-exam-border px-2 py-1 text-[11px] font-semibold text-exam-text-muted hover:bg-exam-border/30"
          >
      ...
      {!outline ? (
        <div className="text-sm text-exam-text-muted italic">
          Chưa có dàn ý cho bản này.
          {other && (
            <button
              type="button"
              onClick={() => setLevel(level === "b1" ? "b2" : "b1")}
              className="ml-2 not-italic font-semibold text-exam-accent underline"
            >
        ...
        <>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {activeGroup.items.map((it) => {
                    ...
                    return (
                      <p key={it.no} className="text-sm leading-loose text-exam-text">
                        <span className="text-[11px] font-semibold text-exam-accent mr-1">
                          {it.no}. {it.label}:
                        </span>
                        {before}
                        <select
          ...
          <div className="mt-5 bg-emerald-50 dark:bg-emerald-950/30 rounded-lg p-4">
            ...
            <p className="text-sm leading-relaxed text-exam-text">{composed}</p>
