...
const TimerDisplay = ({
  ...
  const showPause = !!onTogglePause && !hideTimer;
    ...
    <div className="exam-timer-display flex items-center justify-end gap-2">
      {showPause && (
        <button
          ...
          aria-label={isPaused ? "Tiếp tục tính giờ" : "Tạm dừng tính giờ"}
      ...
      <div
        className={`text-right rounded-md px-2 py-1 border transition-colors ${
          isPaused ? "border-amber-500 bg-amber-50/60" : "border-transparent"
        }`}
      >
        <div
          className={`font-heading text-2xl font-extrabold tracking-wider leading-none tabular-nums inline-block min-w-[9ch] ${
            isPaused ? "text-foreground/40" : "text-foreground"
          }`}
        >
          {formatTime(timeLeft)}
        </div>
        <div className={`text-[11px] mt-0.5 ${isPaused ? "text-amber-600 font-semibold" : "text-muted-foreground"}`}>
          {isPaused ? "Đã tạm dừng" : "Time remaining"}
        </div>
        {/* Progress bar under timer */}
        <div className="h-[3px] w-full bg-muted rounded-full mt-1 overflow-hidden">
          <div
            className={`h-full rounded-full transition-all duration-1000 ${isPaused ? "bg-amber-500" : "bg-[#230859]"}`}
