...
import { Trophy, RotateCcw, Timer, X } from "lucide-react";
...
const MyMatchingMode = ({ words, onExit }: MyMatchingModeProps) => {
  ...
  const cellClass = (id: string, side: "left" | "right") => {
    ...
    if (matched) {
      cls = "border-[hsl(142,60%,45%)] bg-[hsl(142,60%,95%)] dark:bg-[hsl(142,40%,15%)] text-[hsl(142,60%,30%)] dark:text-[hsl(142,60%,70%)] opacity-40 cursor-default";
    } else if (selected) {
      ...
      <div className="border-b border-border bg-card">
        <div className="section-container py-4 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0 text-center">
            <h2 className="font-heading font-bold text-lg text-foreground truncate">Matching</h2>
          </div>
          <Badge className="bg-[hsl(170,55%,40%)] hover:bg-[hsl(170,55%,40%)] text-white text-sm gap-1.5 min-w-[88px] justify-center">
            <Timer className="w-3.5 h-3.5" />
            {finished ? formatTime(finalTime) : formatTime(seconds)}
          </Badge>
        </div>
      </div>
