...
const PARTS = [
  ...
  { id: "part1" as const, label: "P.1", subtitle: "Word recognition (Câu 1 - 13)" },
  { id: "part2" as const, label: "P.2", subtitle: "Matching information (Câu 14)" },
  { id: "part3" as const, label: "P.3", subtitle: "Short conversations (Câu 15)" },
  { id: "part4" as const, label: "P.4", subtitle: "Monologues (Câu 16 - 17)" },
];
...
const Listening = () => {
  ...
  const handleComplete = async (correct: number, total: number, perQuestion?: any[]) => {
    // Locals first — state may be reset if the learner exits right after submitting.
    ...
    const makeSnap = (highlights: Record<string, string>) => {
      try {
        return buildReviewSnapshot({
          ...
          score: correct, total,
          scaled50, band,
          items: buildListeningItems(partType, engineData, highlights, perQuestion || []),
          raw: { engineData, perQuestion: perQuestion || [], highlights },
        });
      } catch { return null; }
    };
    ...
    // 2) Enrich with AI highlights afterwards (best-effort, 8s timeout).
    try {
      ...
      if (examSetId && testResultId && (items?.length || 0) > 0) {
        const res: any = await Promise.race([
          supabase.functions.invoke("listening-highlight", { body: { exam_set_id: examSetId, items } }),
          new Promise((resolve) => setTimeout(() => resolve({ data: null }), 8000)),
        ]);
        const highlights = ((res?.data || {}) as any).highlights || {};
        if (Object.keys(highlights).length > 0) {
          const enriched = makeSnap(highlights);
          if (enriched) {
                    ...
                    return (
                      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}>
                        <div className="group relative rounded-xl p-5 flex flex-col h-full border-2 border-primary/40 bg-card">
                          ...
                          <p className="text-sm text-muted-foreground mb-1">
                            {isPart1
                              ? `${totalWrongQuestions} câu sai · từ ${wrongSets.length} đề bạn đã làm`
                          ...
                          <div className="flex flex-wrap justify-end gap-2 mt-4">
                            <Button
                              ...
                              onClick={() => guard({ access_tier: maxTier } as any, () => setMarathon({
                                ...
                                wrongQuestionIdsBySet: isPart1 ? wrongMap : undefined,
                                wrongRetrySource: "single",
                              }), { feature: 'marathon', itemKey: crypto.randomUUID(), setIds })}
                              className="gap-1.5 font-semibold"
                            >
                              {wrongLocked ? "Mở khóa" : (isPart1 ? `Ôn ${totalWrongQuestions} câu sai` : `Ôn ${wrongSets.length} đề`)}
